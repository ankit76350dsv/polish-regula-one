...
const upsertEmployeeProfile = async (employeeId, data, actor) => {
  ...
  if (existing) {
    ...
    // Re-derive isBlocked, blockReason, and complianceStatus from the (possibly
    // just-changed) requirement flags and document statuses.
    // OLD: this only ran on document upload (updateDocumentReference), so an
    //      employee marked "requires medical/BHP" but with no document uploaded
    //      kept the schema-default isBlocked=false. The dashboard then showed
    //      them as NOT blocked (Clock-in allowed) even though a required
    //      document was missing — contradicting their Non-Compliant status.
    // NEW: recalculate here too, so saving the profile always leaves the block
    //      and compliance state consistent with the required documents.
    recalculateComplianceState(existing);
    ...
    // Build the document in memory first so we can derive its block/compliance
    // state BEFORE the first save — a brand-new employee who requires a document
    // but has not uploaded it must be saved already blocked, not with the
    // schema-default isBlocked=false.
    const newProfile = new SafeWorkEmployee({
      ...complianceData,
      employeeId,
    ...
    recalculateComplianceState(newProfile);
...
const updateEmployeeCompliance = async (employeeId, tenantId, updates, actor) => {
  ...
  profile.updatedBy = actor.userId;
  // Keep block/compliance state consistent with the updated document statuses
  // and requirement flags — same reason as in upsertEmployeeProfile.
  recalculateComplianceState(profile);
  await profile.save();

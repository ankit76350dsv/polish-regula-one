...
export default function Placeholder() {
  ...
  const name = pathname.split("/").filter(Boolean).join(" › ") || "Home";
  ...
  return (
    <div className="min-h-[70vh] flex flex-col items-center justify-center text-center px-4">
      ...
      <p className="text-emerald-600 text-xs font-semibold tracking-widest uppercase mb-3">Page Preview</p>
      <h1 className="text-3xl sm:text-4xl font-bold text-slate-900 capitalize mb-4">{name}</h1>
      <p className="text-slate-500 max-w-md mb-8">
        This page is under construction. Content for <span className="text-slate-700 font-medium capitalize">{name}</span> will be available soon.
      </p>
      <Link
        to="/"

...
exports.isAuthenticatedUser = catchAsyncError(async (req, res, next) => {
  ...
  //
  // Note this is deliberately NOT copied from the local SafeWork user document:
  // a stale local copy could grant access that the platform has already removed.
  ...
  // (for example EMPLOYEE_READ, DOCUMENT_WRITE). The table that decides this
...
// The old check (authorizeRoles, kept below) compared the user's ROLE — for
// example "ROLE_ADMIN" — against a list. A role is far too broad: every tenant
// admin on the whole platform has ROLE_ADMIN, including people who were never
// granted SafeWork at all. So a KSeFFlow-only admin could read SafeWork medical
// and BHP records. That is a real privacy problem under GDPR/RODO, because that
// data is health related.
//
// RegulaOne now returns a per-module "permissions" list, which says exactly what
// a person was granted. We check that instead. Only the permissions listed in
// config/permissions.js get through; everything else is refused with 403.
//
// Usage:
//   authorizePermissions()                    -> the SafeWork list (normal case)
//   authorizePermissions('SAFEVOICE_ADMIN')   -> narrow a single route further
exports.authorizePermissions = (...allowed) => {
  // Work the required list out ONCE when the server starts, not per request.
  // No arguments means "use the module-wide SafeWork list".
  ...
  return (req, res, next) => {
    ...
    if (!permitted) {
      recordDenial(req, { required }, 'Caller does not hold a SafeWork role');
...
//
// This is the check every route should use. It asks about ONE ACTION, not about
// a job title:
//
//   router.get('/users',  authorizeCapability(CAPABILITIES.EMPLOYEE_READ),  ...)
//   router.put('/employees/:id', authorizeCapability(CAPABILITIES.EMPLOYEE_WRITE), ...)
//
...
// middleware simply asks "was this action granted?". So changing what HR may do
// is a one-line edit to that table — no route file changes, nothing to miss.
//
...
// Exported so other middleware (for example the unblock guard) records refusals
// in exactly the same shape, keeping the audit trail consistent.
 ...
 * SafeWork access, which is too wide for health-related compliance records.
 * Use authorizePermissions() above instead — it checks the per-module
 * permission list that RegulaOne returns.
 */

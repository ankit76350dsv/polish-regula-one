...
const getDashboardOverview = async (tenantId) => {
        ...
        stats: [
          {
            $group: {
              ...
              // Required doc selected but file never uploaded (status MISSING)
              missingDocs: {
                $sum: {
                  $cond: [
                    {
  ...
  const employees = (facetResult?.employees || []).map((emp) => {
    ...
    return {
      ...
      site:         emp.site       || '—',
      medicalStatus: (emp.medicalStatus || 'MISSING').toLowerCase(),
      bhpStatus:     (emp.bhpStatus     || 'MISSING').toLowerCase(),
      overallStatus,

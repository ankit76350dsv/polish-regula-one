...
//
// Matches the toggle in SafeWork, using WorkPulse's indigo colour instead of
// SafeWork's emerald so it fits this app's header.
export default function LanguageToggle({ className = "" }) {
  const { language, change, t } = useTranslation();

  const options = [
    // `short` is what the user sees (PL / EN). `name` is the full language name,
    // used only for the hover tooltip so it reads as a real sentence.

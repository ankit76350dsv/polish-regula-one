...
const useCategoryLabel = () => {
  ...
  return (category) => t(`categories.${category}`, { defaultValue: category });
};

export function AccessDeniedView({ onGoReport }) {
  const { t } = useTranslation();
  return (
...
export function PublicReportPortal({ onSubmitReport }) {
  const { t } = useTranslation();
  const jurisdiction = useJurisdiction();
  const categoryLabel = useCategoryLabel();
  const [step, setStep] = useState(1);
  const [category, setCategory] = useState(ReportCategory.Corruption);
  ...
  const submit = (e) => {
    ...
    onSubmitReport({
      ...
      attachments,
      disclosureMode: isHrHandoff
        ? "HR Handoff"
        : wantsRelay
          ? "Confidential Named"
          : "Anonymous",
      contactVaultRef:
        wantsRelay && relayContact.trim()
          ? `vault://safevoice/contact/${Date.now()}`
          : undefined,
    });
  };
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-6xl mx-auto items-start leading-relaxed">
        ...
        >
          <ol className="flex items-center gap-3 mb-6 bg-slate-100 p-3 rounded-lg border border-slate-205 list-none">
            <li className="flex items-center gap-3">
              <span
                className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-mono font-bold ${step === 1 ? "bg-cyan-600 text-white" : "bg-cyan-50 text-cyan-705 border border-cyan-200"}`}
            ...
            <li className="flex items-center gap-3">
              <span
                className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-mono font-bold ${step === 2 ? "bg-cyan-600 text-white" : "bg-slate-200 text-slate-600"}`}
                aria-current={step === 2 ? "step" : undefined}
              >
                2
              </span>
              <span className="text-xs text-slate-500 font-semibold">
                {t("report.stepProtection")}
              </span>
            </li>
          </ol>
            ...
            {step === 1 ? (
              <div className="space-y-5">
                ...
                <div className="flex flex-col gap-1.5">
                  <label
                    htmlFor="incident_desc"
                    className="text-xs font-bold text-slate-700 uppercase font-mono"
                  >
                    {t("report.factsLabel")}
                  </label>
                  <textarea
              ...
              <div className="space-y-5">
                {!isHrHandoff && (
                  <div className="space-y-4">
                    <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4 text-xs text-emerald-805 flex items-start gap-3">
                      ...
                      <div>
                        <span className="font-bold block mb-1">
                          {t("report.anonymousDefaultTitle")}
                        </span>
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 text-xs text-slate-700 space-y-2">
                  <p className="font-semibold text-slate-900">
                    {t("report.privacyNoticeTitle")}
                  </p>
                  <p>
                    {t("report.privacyNoticeController", {
                  ...
                  <p>
                    ...
                    <a
                      href={jurisdiction.externalAuthority.url}
                      target="_blank"
      ...
      <div className="space-y-6">
        <SecureCard
          title={t("report.safeguardsTitle")}
          subtitle={t("report.safeguardsSubtitle")}
        >
          <ul className="space-y-3 text-xs text-slate-700">
            {[
              t("report.safeguard1", {
                days: jurisdiction.acknowledgementDays,
                months: jurisdiction.feedbackMonths,
              }),
              t("report.safeguard2"),
              t("report.safeguard3"),
              t("report.safeguard4", {
                days: jurisdiction.irrelevantDataDeletionDays,
              }),
              t("report.safeguard5"),
            ].map((item) => (
        ...
        <SecureCard
          title={t("report.minimisationTitle")}
          subtitle={t("report.minimisationSubtitle")}
        >
...
export function ReportSuccessView({ generatedCode, pin, category }) {
  const { t } = useTranslation();
    ...
    <div className="max-w-xl mx-auto text-center py-6 leading-relaxed">
      ...
      {isHrHandoff ? (
        <div className="bg-amber-50 border border-amber-250 rounded-lg p-6 mt-8 text-left text-xs text-slate-700 space-y-3">
          <p className="font-bold text-amber-855">
            {t("success.hrNoCodeTitle")}
          </p>
        ...
        <div className="space-y-6 mt-8">
          ...
          <div
            className="bg-rose-50 border border-rose-200 text-rose-700 text-xs rounded-lg p-4 text-left leading-normal"
            role="note"
          >
...
export function TrackCaseView({
  ...
  const handleTrackSubmit = (e) => {
    ...
    const matched = reports.find(
      (report) => report.trackingCode === codeInput.trim(),
    );
    if (matched) {
  ...
  const currentCaseMessages = trackedCase
    ? messages.filter((msg) => msg.caseId === trackedCase.id)
    : [];
        ...
        >
          <form onSubmit={handleTrackSubmit} className="space-y-4">
            ...
            {errorText && (
              <div
                role="alert"
                className="text-xs text-rose-700 bg-rose-50 p-2.5 rounded border border-rose-200"
              >
                {errorText}
              </div>
            )}
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start max-w-6xl mx-auto leading-relaxed">
      <div className="lg:col-span-2 space-y-6">
        <div className="flex items-center justify-between pb-4 border-b border-slate-200">
          <div>
            <span className="text-[10px] font-mono uppercase text-slate-500 block">
              {t("track.anonymousCase")}
            </span>
            <h1 className="text-md font-mono text-slate-800 font-bold">
              {trackedCase.id}
            </h1>
          </div>
        ...
        <SecureCard
          ...
          subtitle={t("track.deadlines", {
            ack: trackedCase.acknowledgementDue,
            feedback: trackedCase.feedbackDue,
          })}
        >
        ...
        <SecureCard
          title={t("track.addEvidence")}
          subtitle={t("track.addEvidenceSub")}
        >
          <AttachmentUploader
            files={evidenceFiles}
      ...
      <div className="space-y-6">
        <SecureCard
          title={t("track.secureConversation")}
          subtitle={t("track.secureConversationSub")}
          isEncrypted
        >
          <div className="max-h-80 overflow-y-auto space-y-3 min-h-48">
            {currentCaseMessages.length === 0 ? (
              ...
              currentCaseMessages.map((msg) => (
                <ChatBubble
                  key={msg.id}
                  sender={msg.sender === "Reporter" ? "Reporter" : msg.sender}
                  text={msg.text}
          ...
          >
            <label htmlFor="track_message" className="sr-only">
              {t("track.messagePlaceholder")}
            </label>
            <input
              id="track_message"
              type="text"
            ...
            <button
              ...
              className="bg-cyan-600 hover:bg-cyan-700 text-white p-2 rounded-lg transition-colors cursor-pointer focus:outline-none focus:ring-2 focus:ring-cyan-500"
            >
        ...
        <SecureCard title={t("track.metadataPolicy")}>
          <div className="grid gap-2 text-xs">
            {Object.entries(trackedCase.technicalMetadataPolicy).map(
              ([key, value]) => (
                <div
                  key={key}
                  className="flex items-center justify-between rounded bg-slate-50 border border-slate-200 px-3 py-2"
                >
                  <span className="text-slate-700">
                    {key.replace(/([A-Z])/g, " $1")}
                  </span>
                  <span className="text-emerald-700 font-mono uppercase">
                    {value ? t("track.stored") : t("track.notStored")}
                  </span>
                </div>
              ),
            )}
...
export function AdminDashboard({ reports, activeRole, onNavigateToCases }) {
  const { t } = useTranslation();
  const jurisdiction = useJurisdiction();
  ...
  const ackDue = reports.filter(
    (report) => report.status === "Received",
  ).length;
  const legalHolds = reports.filter(
    (report) => report.retention.state === "Legal Hold",
  ).length;
  const anonymousCount = reports.filter(
    (report) => report.disclosureMode === "Anonymous",
  ).length;
  const deletionScheduled = reports.filter(
    (report) => report.retention.state === "Deletion Scheduled",
  ).length;
    ...
    <div className="space-y-8 leading-relaxed">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        {[
          ...
          {
            label: t("dashboard.anonymous"),
            val: anonymousCount,
            icon: Shield,
          },
          {
            label: t("dashboard.deletionScheduled"),
            val: deletionScheduled + legalHolds,
            icon: Archive,
          },
        ].map((item) => {
          ...
          return (
            <div
              key={item.label}
              className="bg-white border border-slate-205 rounded-lg p-4 flex flex-col justify-between shadow-xs"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <SecureCard
          title={t("dashboard.workflowTitle")}
          subtitle={t("dashboard.workflowSubtitle")}
        >
          <ul className="space-y-3 text-xs text-slate-700">
            {[
              t("dashboard.workflow1", {
                days: jurisdiction.acknowledgementDays,
              }),
              t("dashboard.workflow2", { months: jurisdiction.feedbackMonths }),
              t("dashboard.workflow3"),
              t("dashboard.workflow4", {
                days: jurisdiction.irrelevantDataDeletionDays,
              }),
        ...
        <SecureCard
          ...
          subtitle={t("dashboard.analyticsSubtitle")}
        >
          <p className="text-xs text-slate-700">
            {t("dashboard.analyticsBody")}
          </p>
        </SecureCard>

        <SecureCard title={t("dashboard.nextAction")}>
          <p className="text-xs text-slate-700 mb-4">
            {t("dashboard.nextActionBody")}
          </p>
          <AppButton
            variant="primary"
...
export function CaseManagementGrid({ reports, activeRole, onSelectCase }) {
  const { t } = useTranslation();
  const categoryLabel = useCategoryLabel();
  ...
  const filtered = reports.filter((report) => {
    const searchable =
      `${report.id} ${report.category} ${report.department} ${report.status}`.toLowerCase();
    ...
    <div className="space-y-6 leading-relaxed">
      <div className="flex flex-col md:flex-row items-center gap-4 bg-slate-50 p-4 rounded-lg border border-slate-200">
        <div className="relative w-full md:flex-1">
          <label htmlFor="case_search" className="sr-only">
          ...
          <Search
            className="w-4 h-4 text-slate-550 text-slate-500 absolute left-3 top-2.5"
            aria-hidden="true"
          />
        </div>
        <label htmlFor="case_status_filter" className="sr-only">
          {t("cases.headers.status")}
        </label>
        <select
      ...
      <AppTable
        headers={[
          t("cases.headers.caseId"),
          t("cases.headers.category"),
          t("cases.headers.status"),
          t("cases.headers.deadlines"),
          t("cases.headers.retention"),
          t("cases.headers.assigned"),
          t("cases.headers.disclosure"),
          t("cases.headers.action"),
        ]}
      >
        {filtered.length === 0 ? (
          <tr>
          ...
          filtered.map((caseItem) => (
            ...
            >
              ...
              <td className="px-4 py-3 text-[11px] text-slate-500 font-mono">
                <div>
                  {t("cases.ack")}: {caseItem.acknowledgementDue}
                </div>
                <div>
                  {t("cases.feedback")}: {caseItem.feedbackDue}
                </div>
              </td>
              <td className="px-4 py-3 text-xs">
                <span className="bg-slate-100 px-2 py-1 rounded border border-slate-200 text-slate-700">
                  {caseItem.retention.state}
                </span>
              </td>
              <td className="px-4 py-3 text-xs text-slate-700">
                {caseItem.assignedInvestigator || t("cases.unassigned")}
              </td>
              <td className="px-4 py-3 text-xs text-cyan-700">
                {caseItem.disclosureMode}
              </td>
              <td className="px-4 py-3">
                <AppButton
                  size="sm"
                  variant="outline"
                  aria-label={`${t("common.open")} ${caseItem.id}`}
...
export function CaseDetailsView({
  ...
  const { t } = useTranslation();
  const categoryLabel = useCategoryLabel();
  const [stateStatus, setStateStatus] = useState(caseItem.status);
  const [stateSeverity, setStateSeverity] = useState(caseItem.severity);
  const [stateInv, setStateInv] = useState(caseItem.assignedInvestigator || "");
  const [noteText, setNoteText] = useState("");
  const [adminMsg, setAdminMsg] = useState("");
  const [holdReason, setHoldReason] = useState(
    caseItem.retention.legalHoldReason || "",
  );
  ...
  const currentCaseMessages = messages.filter(
    (msg) => msg.caseId === caseItem.id,
  );
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 items-start leading-relaxed text-slate-700">
      <div className="xl:col-span-2 space-y-6">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between pb-5 border-b border-slate-200 gap-4">
          <div>
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-mono select-all bg-slate-100 px-2 py-0.5 rounded border border-slate-200 text-slate-700">
                {caseItem.id}
              </span>
              <CaseStatusBadge status={caseItem.status} />
              <CaseSeverityBadge severity={caseItem.severity} />
            </div>
            <h1 className="text-md font-bold text-slate-900 mt-2">
              {categoryLabel(caseItem.category)}
            </h1>
          </div>
          <div className="text-xs bg-slate-50 px-4 py-2 rounded-lg border border-slate-200">
            <span className="text-slate-500 font-mono">
              {t("caseDetails.feedbackDue")}{" "}
            </span>
            <span className="font-mono text-emerald-700 font-bold">
              {caseItem.feedbackDue}
            </span>
          </div>
        </div>

        <SecureCard
          title={t("caseDetails.actionCenter")}
          subtitle={t("caseDetails.actionCenterSub")}
        >
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
            <div>
              <label
                htmlFor="case_status"
                className="text-[11px] font-bold text-slate-650 uppercase font-mono block mb-1"
              >
                {t("caseDetails.statusLabel")}
              </label>
              <select
                id="case_status"
                value={stateStatus}
              ...
              >
                {statuses.map((status) => (
                  <option
                    key={status}
                    value={status}
                    disabled={status === "Closed" && !canClose}
                  >
                    {t(`status.${status}`)}
                  </option>
                ))}
            ...
            <div>
              <label
                htmlFor="case_severity"
                className="text-[11px] font-bold text-slate-655 text-slate-650 uppercase font-mono block mb-1"
              >
                {t("caseDetails.severityLabel")}
              </label>
              <select
                id="case_severity"
                value={stateSeverity}
                onChange={(e) => setStateSeverity(e.target.value)}
                className="bg-white w-full text-xs font-semibold text-slate-800 border border-slate-300 rounded px-3.5 py-2.5 outline-none cursor-pointer focus:border-cyan-500"
              >
            ...
            <div>
              <label
                htmlFor="case_investigator"
                className="text-[11px] font-bold text-slate-650 uppercase font-mono block mb-1"
              >
                {t("caseDetails.investigatorLabel")}
              </label>
              <select
        ...
        <SecureCard
          title={t("caseDetails.synopsis")}
          subtitle={t("caseDetails.synopsisSub")}
        >
          ...
          {caseItem.attachments.length > 0 && (
            <div className="pt-4 mt-4 border-t border-slate-200">
              <span className="text-[10px] font-bold text-slate-500 font-mono uppercase block mb-2">
                {t("caseDetails.evidenceRefs", {
                  count: caseItem.attachments.length,
                })}
              </span>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {caseItem.attachments.map((file) => (
                  <div
                    key={file.id}
                    className="bg-slate-50 px-3 py-2.5 rounded-lg border border-slate-200 text-xs font-mono"
                  >
                    <div className="flex items-center justify-between gap-2">
                      <span className="truncate flex items-center gap-2 text-slate-700">
                        <FileText
                          className="w-4 h-4 text-emerald-605 text-emerald-600 shrink-0"
                          aria-hidden="true"
                        />{" "}
                        {file.displayName}
                      </span>
        ...
        <SecureCard
          title={t("caseDetails.timeline")}
          subtitle={t("caseDetails.timelineSub")}
        >
      ...
      <div className="space-y-6">
        <SecureCard
          title={t("caseDetails.retentionTitle")}
          subtitle={t("caseDetails.retentionSub", {
            date: caseItem.retention.deleteAfter,
          })}
        >
          <div className="space-y-3 text-xs">
            <div className="flex items-center justify-between rounded bg-slate-50 border border-slate-200 px-3 py-2">
              ...
              <span className="text-cyan-705 text-cyan-705 text-cyan-700">
                {caseItem.retention.state}
              </span>
            </div>
            <div className="flex items-center justify-between rounded bg-slate-50 border border-slate-200 px-3 py-2">
              ...
              <span className="text-slate-700">
                {caseItem.retention.irrelevantPersonalDataDeletionDue}
              </span>
            </div>
            <SecureTextField
            ...
            <div className="flex gap-2">
              <AppButton
                ...
                variant="secure"
                disabled={!canRetention}
                onClick={() => onRetentionUpdate(caseItem.id, true, holdReason)}
              >
              ...
              <AppButton
                ...
                variant="outline"
                disabled={!canRetention}
                onClick={() => onRetentionUpdate(caseItem.id, false)}
              >
        ...
        <SecureCard title={t("caseDetails.anonymousMessages")} isEncrypted>
          ...
          <form
            ...
            className="mt-4 pt-4 border-t border-slate-200 flex gap-2"
          >
            <label htmlFor="admin_reply" className="sr-only">
              {t("caseDetails.replyPlaceholder")}
            </label>
            <input
              id="admin_reply"
              type="text"
            ...
            <button
              type="submit"
              aria-label={t("track.sendMessage")}
              className="bg-cyan-600 hover:bg-cyan-700 text-white p-2.5 rounded-lg shrink-0 transition-colors cursor-pointer focus:outline-none focus:ring-2 focus:ring-cyan-500"
            >
              <Send className="w-4 h-4" />
            </button>
          </form>
        ...
        <SecureCard title={t("caseDetails.restrictedNote")}>
          <form
            ...
            className="space-y-3"
          >
            <label htmlFor="restricted_note" className="sr-only">
              {t("caseDetails.restrictedNote")}
            </label>
            <textarea
              id="restricted_note"
              value={noteText}
...
export function CentralEncryptedInbox({
  ...
  const trackableCases = reports.filter((report) =>
    Boolean(report.trackingCode),
  );
  const selectedCase = trackableCases[activeCaseIdx] || trackableCases[0];
  const selectedCaseMessages = selectedCase
    ? messages.filter((msg) => msg.caseId === selectedCase.id)
    : [];
    ...
    <div className="border border-slate-200 rounded-lg overflow-hidden bg-white grid grid-cols-1 md:grid-cols-3 max-w-5xl mx-auto h-[550px] leading-relaxed shadow-lg">
      <div className="border-r border-slate-200 bg-slate-50 overflow-y-auto divide-y divide-slate-200 h-full">
        <div className="p-4 border-b border-slate-200 bg-slate-100/50 flex items-center justify-between">
          ...
          <span className="text-[10px] text-cyan-700 bg-cyan-50 px-2 py-0.5 rounded border border-cyan-200 font-mono font-bold">
            {trackableCases.length}
          </span>
        </div>
        {trackableCases.map((report, idx) => (
          <button
            ...
            className={`w-full text-left p-4 hover:bg-slate-100 transition-colors cursor-pointer flex flex-col gap-1.5 focus:outline-none focus:ring-2 focus:ring-cyan-500 ${
              activeCaseIdx === idx
                ? "bg-slate-100 border-l-4 border-cyan-600"
                : ""
            }`}
          >
      ...
      <div className="md:col-span-2 flex flex-col justify-between h-full">
        ...
        >
          <label htmlFor="inbox_reply" className="sr-only">
            {t("inbox.replyTo", { id: selectedCase.id })}
          </label>
          <input
            id="inbox_reply"
            type="text"
...
export function SecurityAuditTrailLogs({ logs, activeRole }) {
  const { t } = useTranslation();
  ...
  const filteredLogs = logs.filter((log) => {
    const text =
      `${log.actorRole} ${log.actorRef} ${log.actionType} ${log.subjectId || ""} ${log.metadataNotice}`.toLowerCase();
    ...
    <div className="space-y-6 leading-relaxed max-w-5xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row items-center gap-4 bg-white p-4 rounded-lg border border-slate-200 justify-between shadow-xs">
        <div className="relative w-full md:max-w-xs">
          <label htmlFor="audit_search" className="sr-only">
            {t("audits.searchPlaceholder")}
          </label>
          <input
            id="audit_search"
            type="text"
      ...
      <AppTable
        headers={[
          t("audits.headers.timestamp"),
          t("audits.headers.actor"),
          t("audits.headers.action"),
          t("audits.headers.subject"),
          t("audits.headers.outcome"),
          t("audits.headers.metadata"),
          t("audits.headers.seal"),
        ]}
      >
...
export function UsersPermissionsMatrix({ users, activeRole, onInviteUser }) {
  const { t } = useTranslation();
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto leading-relaxed">
      ...
      <SecureCard title={t("users.authorizedPersonnel")}>
        <AppTable
          headers={[
            t("users.headers.officer"),
            t("users.headers.role"),
            t("users.headers.status"),
            t("users.headers.mfa"),
            t("users.headers.lastReview"),
          ]}
        >
          {users.map((user) => (
            <tr
              key={user.id}
              className="hover:bg-slate-50 border-b border-slate-200 text-xs"
            >
      ...
      <SecureCard
        title={t("users.permissionMatrix")}
        subtitle={t("users.permissionMatrixSub")}
      >
        <AppTable
          headers={[
            t("users.headers.role"),
            t("users.headers.view"),
            t("users.headers.assign"),
            t("users.headers.close"),
            t("users.headers.export"),
            t("users.headers.audits"),
            t("users.headers.usersCol"),
            t("users.headers.retention"),
          ]}
        >
            ...
            >
              ...
              {[
                ...
                <td key={key} className="px-4 py-3 text-center text-xs">
                  ...
                  >
                    {rule[key] ? t("users.allowed") : t("users.blocked")}
                  </span>
                </td>
              ))}
            </tr>
...
export function BrandedSettingsView() {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 max-w-6xl mx-auto items-start leading-relaxed">
      <div
        className="bg-white border border-slate-200 p-3 rounded-lg space-y-1"
        role="tablist"
        aria-label={t("nav.complianceSettings")}
      >
        {tabs.map((tab) => {
          const Icon = tab.icon;
          return (
      ...
      <div className="lg:col-span-3">
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
              {[
                ...
                [
                  "Session expiry",
                  "15 minute idle timeout; 8 hour absolute maximum",
                ],
                [
                  "Session revocation",
                  "Immediate revocation for role change, departure, or suspected compromise",
                ],
                [
                  "Password policy",
                  "OIDC preferred; local fallback requires strong password and breach checks",
                ],
                [
                  "Login monitoring",
                  "Security team only; do not expose source IP to case investigators",
                ],
                [
                  "Account lockout",
                  "Progressive lockout and alerting after repeated failures",
                ],
                [
                  "Rate limiting",
                  "Public tracking and intake endpoints must be rate-limited without fingerprinting",
                ],
                [
                  "No telemetry",
                  "No analytics, marketing pixels, or device/browser fingerprinting",
                ],
              ].map(([label, value]) => (
                <div
                  key={label}
        ...
        {activeSettingsTab === "retention" && (
          <SecureCard
            ...
            subtitle={t("settings.retentionSub")}
          >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-slate-50 border border-slate-200 rounded-lg p-3">
                  ...
                  <div className="text-slate-600 mt-1">
                    {t("settings.defaultRetentionBody", {
                      years: jurisdiction.retentionYears,
                    })}
                  </div>
                </div>
                <div className="bg-slate-50 border border-slate-200 rounded-lg p-3">
                  <div className="font-bold text-slate-800">
                    {t("settings.irrelevantData")}
                  </div>
                  <div className="text-slate-600 mt-1">
                    {t("settings.irrelevantDataBody", {
                      days: jurisdiction.irrelevantDataDeletionDays,
                    })}
                  </div>
                </div>
                <div className="bg-slate-50 border border-slate-200 rounded-lg p-3">
                  <div className="font-bold text-slate-800">
                    {t("settings.legalHold")}
                  </div>
                  <div className="text-slate-600 mt-1">
                    {t("settings.legalHoldBody")}
                  </div>
                </div>
                <div className="bg-slate-50 border border-slate-200 rounded-lg p-3">
                  <div className="font-bold text-slate-800">
                    {t("settings.secureDestruction")}
                  </div>
                  <div className="text-slate-600 mt-1">
                    {t("settings.secureDestructionBody")}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4 text-xs text-slate-700">
              <div className="bg-slate-50 p-4 border border-slate-200 rounded-lg">
                <div className="flex items-center gap-1.5 font-bold text-slate-800 mb-2">
                  ...
                  {t("settings.processingBasis")}
                </div>
                <p className="text-slate-600">
                  {t("settings.processingBasisBody")}
                </p>
              </div>
              <div className="bg-slate-50 p-4 border border-slate-200 rounded-lg">
                <div className="flex items-center gap-1.5 font-bold text-slate-800 mb-2">
                  <UserCheck
                    ...
                    aria-hidden="true"
                  />{" "}
                  {t("settings.responsibilities")}
                </div>
                <p className="text-slate-600">
                  {t("settings.responsibilitiesBody")}
                </p>
              </div>
            </div>
          </SecureCard>
        ...
        {activeSettingsTab === "review" && (
          <SecureCard
            ...
            subtitle={t("settings.reviewSub")}
          >
            <AppTable
              headers={[
                t("settings.reviewHeaders.area"),
                t("settings.reviewHeaders.feature"),
                t("settings.reviewHeaders.decision"),
                t("settings.reviewHeaders.justification"),
                t("settings.reviewHeaders.risk"),
              ]}
            >
              {reviewGroups.map((item) => (
                <tr
                  key={`${item.area}-${item.classification}`}
                ...
                >
                  ...
                  <td className="px-4 py-3 text-xs text-amber-800 font-semibold">
                    {item.risk}
                  </td>
                </tr>
              ))}

...
interface OfflineQueueProps {
  ...
  onProcessOfflineItem: (invoiceId: string, success: boolean, newKsefId?: string) => void;
  onAddNotification: (title: string, message: string, type: 'info' | 'success' | 'warn' | 'error') => void;
}

export default function OfflineQueue({ tenant, role, invoices, govStatus, onSetGovStatus, onProcessOfflineItem, onAddNotification }: OfflineQueueProps) {
  ...
  // Automatic scheduler countdown simulation
  const [secondsLeft, setSecondsLeft] = useState(30);
  const [isProcessingCollection, setIsProcessingCollection] = useState(false);

  // Active ticking countdown when queue is not empty
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setSecondsLeft((prev) => {
        if (prev <= 1) {
          // Trigger queue auto-flush attempt!
  ...
  // Automated Flush process checking active connection
  const triggerAutomatedFlush = () => {
    ...
    if (govStatus === 'Connected') {
      ...
      setTimeout(() => {
        // Sync and flush all items in state
      ...
      // Failed sync because api still down
      offlineInvoices.forEach(inv => {
        onProcessOfflineItem(inv.id, false); // Increment attempts
      });
      onAddNotification(
  ...
  const triggerManualBypass = () => {
    ...
    // Force connection active for test purposes so flush succeeds
    if (govStatus !== 'Connected') {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 bg-white border border-stone-200/90 rounded-xl p-5 hover:border-red-300 transition-all shadow-xs space-y-5">
          ...
          <div className="space-y-4">
            {/* Realtime Countdown status visual */}

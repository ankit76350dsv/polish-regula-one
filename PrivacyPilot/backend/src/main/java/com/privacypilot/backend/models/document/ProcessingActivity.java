...
public class ProcessingActivity extends BaseDocument {
    ...
    // The fields below are ordered to MATCH the frontend "Add Activity" wizard,
    // so reading this class top-to-bottom follows the exact order a user fills
    // the form in (Step 1 → Step 8). See ActivityWizardPage.jsx.
    ...
    //       for (Art. 30(2)(a)). Shown on Step 1 only when role = PROCESSOR.
    ...
    //! NOTE: Filled automatically from the logged-in user when the activity is created.
    ...
    //! NOTE: Starts as DRAFT automatically; not typed on Step 1.
    ...
    // !COLLECTED? YES — a dropdown that appears on Step 3 ONLY when the user picks a
    //            special-category data type; it is required once shown.
    private Art9Condition art9Condition;

    // WHAT: True if this activity uses criminal-conviction / offence data (Art. 10).
    // WHY: Criminal data has special rules.
    // EXAMPLE: true for a background-check activity; false for a newsletter.
    //! COLLECTED? NO — there is no checkbox for this. The app sets it automatically
    //            at save time from whether the "criminal" data category was ticked.
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // NOT COLLECTED IN THE WIZARD — set by the system / later workflow
    // ─────────────────────────────────────────────────────────────────────────

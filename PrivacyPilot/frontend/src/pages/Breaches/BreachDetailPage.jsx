...
export default function BreachDetailPage() {
  ...
  useEffect(() => {
    ...
    if (remainingMs <= REMIND_THRESHOLD_MS) {
      ...
      const msg = remainingMs <= 0
        ? t('breach.reminderElapsed')
        : t('breach.reminderSoon').replace('{hours}', Math.ceil(remainingMs / 3600000));
  ...
  // A name a person can read, built from the report kind and the breach title. It used to
  // embed the raw database id: "UODO_breach_report_6a68354b91d0405a8b2d6007".
  const reportName = (extension) =>
    documentFilename(t('breach.report33'), breach.title, null, extension);
  ...
  const notifyUodo = async () => {
    ...
    if (action.error) toast.error(failureMessage(action.error, t));
    else toast.success(t('breach.uodoNotified'));
  };
    ...
    <div className="mx-auto max-w-3xl">
      ...
      <div className="grid gap-4">
          ...
          <Card className={remaining <= 0 ? 'border-(--status-risk)/60' : 'border-(--status-warn)/50'}>
            <CardContent className="flex flex-wrap items-center gap-4">
              <div>
                ...
                {/* The old note pointed at uodo.gov.pl. A breach notification is in fact
                    submitted electronically and must be signed with a qualified electronic
                    signature or a trusted profile — which is what a user needs to know
                    before they try to file it. */}
                <p className="mt-1 max-w-prose text-xs text-muted-foreground">
                  {t('breach.notifyDeadlineNote')}
                </p>
              </div>
              {canManage && (
                <div className="ml-auto flex flex-wrap gap-2">
                  ...
                  <Button onClick={notifyUodo}>
                    <CheckCircle2 /> {t('breach.markNotified')}
                  </Button>
                </div>
              )}
            </CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">{t('breach.details')} — Art. 33(3)</CardTitle>
          </CardHeader>
          <CardContent className="grid gap-2 text-sm">
            ...
            <div className="mt-2 grid gap-1 text-xs text-muted-foreground">
              ...
              <span>{t('breach.subjects')}: {breach.subjectsCount}</span>
              ...
              {/* The stored code ("medium") was printed straight to the screen. */}
              <span>{t('risk.level')}: {breach.riskLevel ? t(`risk.${breach.riskLevel}`) : '—'}</span>
              <span>
              ...
              {breach.uodoNotifiedAt && (
                <span className="flex items-center gap-1.5 text-(--status-ok)">
                  ...
                  {t('breach.uodoNotified')}: {new Date(breach.uodoNotifiedAt).toLocaleString(lang === 'pl' ? 'pl-PL' : 'en-GB')}
                </span>
              )}
        ...
        {breach.uodoNotificationRequired && (
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm">{t('breach.uodoRef')}</CardTitle>
            </CardHeader>
            <CardContent>
              {canManage ? (
                <div className="flex flex-wrap gap-2">
                  <Input value={reference} onChange={(e) => setReference(e.target.value)}
                    ...
                    placeholder="DKN.5131.2026.XYZ"
                    aria-label={t('breach.uodoRef')} className="max-w-xs" />
              ...
              <p className="mt-1.5 text-xs text-muted-foreground">{t('breach.uodoRefHint')}</p>
            </CardContent>
          </Card>
        )}
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">{t('breach.remediation')} — Art. 33(3)(d)</CardTitle>
          </CardHeader>
          <CardContent className="grid gap-1.5">
            {breach.remediation.map((r) => (
              <label key={r.id} className="flex cursor-pointer items-center gap-2 rounded-lg border px-3 py-2 text-sm hover:bg-accent">
                <input type="checkbox" className="accent-primary" checked={r.done}
            ...
            {breach.remediation.length === 0 && (
              <p className="text-sm text-muted-foreground">{t('breach.noRemediation')}</p>
            )}
            {canManage && (
      ...
      <Dialog open={reportOpen} onOpenChange={setReportOpen}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle>{t('breach.reportTitle')}</DialogTitle>
            <DialogDescription>{t('breach.reportBody')}</DialogDescription>
          </DialogHeader>
          ...
          <DraftsDisclaimer />
          {!settings.data && (
            <p className="text-xs text-(--status-warn)">{t('breach.reportSettingsMissing')}</p>
          )}
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button variant="outline" size="sm" onClick={async () => {
              if (await recordReportExport('markdown')) {
                downloadMarkdown(reportName('md'), report);
              }
            }}>
              <Download /> Markdown
            </Button>
            <Button variant="outline" size="sm" onClick={async () => {
              if (await recordReportExport('word')) {
                downloadWord(reportName('doc'), breach.title, report);
              }
            }}>
              <Download /> Word
            </Button>

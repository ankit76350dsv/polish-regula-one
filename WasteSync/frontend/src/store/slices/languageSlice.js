...
//
// This mirrors SafeWork's languageSlice on purpose, so a person moving between
// the two RegulaOne apps finds the same switch behaving the same way.
...
//
// The key is prefixed with "wastesync." so it can never clash with the key
// SafeWork uses ("safework.language"). The apps may sit on the same domain, and
// one app quietly changing the other app's language would be very confusing.
const STORAGE_KEY = "wastesync.language";

// Polish is the DEFAULT because WasteSync files BDO reports with a Polish
// government register and is used by Polish staff. English is the second option.

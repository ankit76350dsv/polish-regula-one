...
//
// ModuleAccessGuard already answered the bigger questions (is WasteSync in your
// package, is the plan paid, were you given WasteSync at all). This guard answers
// the last one: does this particular page match what you may do?
//
// Example: an HR manager may record waste figures, but must NOT open the Audit
// Logs page (reading the audit trail is the auditor's job). Wrapping that route in
// this guard means HR sees one clear message instead of an empty page full of
// failed requests.
//
// Usage in the router:
//   {
...
export default function RequireCapability({ capability, anyOf, children }) {
  ...
  if (!allowed) {
    // A different message from "you have no WasteSync access": this user DOES use
    // WasteSync, just not this particular page.

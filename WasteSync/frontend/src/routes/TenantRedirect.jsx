...
//   2. An old flat bookmark:    "/reports/123"   -> "/company/{tenantId}/reports/123"
//
// WHY CASE 2 EXISTS (BACKWARD COMPATIBILITY)
// Before this change every page lived at a flat address like "/reports". People
...
export default function TenantRedirect() {
  ...
  //   "/"          -> keep nothing, land on the home (dashboard) page.
  //   "/company"   -> a half-typed tenant address; the id is missing, so we also
  //                   just send them home.
  // Note we test "/company/" WITH the slash on purpose: "/companies" is a real
  // page of ours and must be kept, and it also starts with the word "company".
  const isRoot = path === "/";
  const isBareCompanyPath = path === "/company" || path === "/company/";
  const subPage = isRoot || isBareCompanyPath ? HOME_SUBPATH : path;

  // Keep the query string (year filters, and so on) so a shared link still opens

...
//   ModuleAccessGuard   -> "may you use WasteSync?" (licence, plan, permission)
...
//   /company/MY-COMPANY/reports  ->  /company/SOMEONE-ELSE/reports
// When that happens we quietly put them back on their OWN company, keeping the
// page they were trying to reach (…/reports stays …/reports) and any filters in
...
// another company's figures even if this check were removed. This guard only
// keeps the SCREEN honest, so the address bar can always be trusted to say which
// company the numbers on it belong to.
export default function TenantGuard() {
  ...
  // A signed-in user with no company at all cannot have a WasteSync licence, so
  ...
  if (tenantIdInUrl !== userTenantId) {
    ...
    // "/company/x/reports/123".split("/") -> ["", "company", "x", "reports", "123"]
    // so dropping the first three parts leaves "reports/123". If they named no page
